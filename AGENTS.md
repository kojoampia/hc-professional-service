...
- `.jhipster/*.json` — JHipster entity definitions, **held in sync with the domain classes by a test rather than by a date** (`JhipsterEntityFieldsTest`, `../docs/backlog.md` item 21). This line read "kept in sync … as of 2026-07-30", which was true on the day it was written and wrong six days later: MOB9 added `Profile`'s three push preferences on 2026-08-05 and DR2 added `DutyRoster.visits` on 2026-08-20, both to the documents and to neither definition file, and the claim stood unchallenged until 2026-09-11. That is what a date-stamped claim is worth in a directory nothing builds from. `DutyRoster` was generated in WP6 (with `patientId` dropped and the enum corrected — the JDL's `MEDIC`/`VENDOR`/`ADMINISTRATOR` values are gone). `Patient.json` was removed: it was a mislabelled copy of an outdated `Profile` (internal `name` was `Profile`), and `Patient` belongs to `patientservice`, a backend not in this workspace.

  **Do not regenerate entities from these definitions without reading this first.** They describe the fields accurately — every field, or an explicit note saying the format cannot state it, both enforced — but regeneration still destroys hand-written code the templates know nothing about:
  ...
  - **Five fields cannot be expressed in this format at all**, and each now says so in its own definition file under `fieldsThisFormatCannotExpress`, with the reason — so the warning is at the point where somebody regenerates rather than only here. JDL's field vocabulary is scalar (`jdl/jhipster/field-types.js`, `CommonDBTypes`): no collections, no embedded value types. Four are stored as `String` approximations — `Profile.address` (embedded `Address`), `Profile.emergencyContact` (embedded `EmergencyContact`), `Profile.teamIds` and `Team.members` (both `List<String>`) — and regenerating those two entities emits `String` and breaks their consumers. The fifth, `DutyRoster.visits` (`List<Visit>`, the DR2 round), is not approximated but **absent**, because a `String` would not have been an approximation of anything; regenerating `DutyRoster` drops the round outright. `fieldsThisFormatCannotExpress` is not a generator option — the generator ignores unknown keys and would drop it on a rewrite — and `JhipsterEntityFieldsTest` holds every entry to naming a real field with a real reason, so it cannot outlive its subject.
...
**`professional.event` carries EVERY entity CRUD event, and it is a third channel rather than a rename** (`../docs/backlog.md` item 141; hc-admin's item 110 for the estate decision behind it — one channel per product, `{patient,professional,vendor,admin}.event`). ⛔ **`hc.professional.entity` and `hc.professional.registration` are untouched and must stay so**: hc-admin consumes the first through `professionalProfileConsumer-in-0`, and this service's own `messageEvents-in-0` and `pushEvents-in-0` read it too. Retiring either is item 142 and is sequenced behind this. `EntityChangeBindingTest` asserts the new destination _and_ the live ones against the **shipped** `config/application.yml`, read from disk — because test resources shadow main ones on the test classpath, so a `ClassPathResource` would assert against a file no deployment ever loads.

Three properties are easy to undo and each has a test:

- **The actor is `SecurityUtils.getCurrentAccountId()` — the gateway `User.id` — and never the login.** This is where the repo-wide rule above inverts in consequence rather than in wording: `lastModifiedBy` on `ProfileStatus` is a login and is fine, because hc-admin renders it and joins it to nothing. Here hc-admin **stores** it, and item 107 D1 makes `accountId` the estate's join key, so a login under that name is a value that looks joinable and is not. **Absent rather than `"system"`** when no account is behind the write — a scheduler, a migration, a consumer, a startup runner. `EntityChangeAnnouncerTest` gives its token a subject and a `uid` that **differ**, because a fixture where they matched would pass against either implementation and prove nothing.
- **It publishes off the caller's thread**, through a single-threaded bounded-queue executor with **`AbortPolicy`**. ⚠ `CallerRunsPolicy` reads as back-pressure and would silently restore the defect: StreamBridge creates its binding inside the first `send()`, which blocks ~60s against an absent broker — and unlike every other publisher here, this one rides a persistence callback, so that cost lands on _every_ write rather than on one endpoint.
- **The payload is five identifiers and no field values** — `entityType`, `entityId`, `action`, the envelope's `occurredAt`, and `actorAccountId`. Asserted as `containsOnlyKeys` rather than as a list of forbidden names, because this callback fires for `Profile` exactly as it fires for everything else, so a value leaking here would be a clinician's.

`service/EntityChangeAnnouncer` is the hook — `AbstractMongoEventListener<Object>`, so **every** collection, including ones generated after it. It carries **no recursion guard, and that is stated rather than forgotten**: unlike hc-admin's `AuditLogCallback`, which must exclude its own writes or the first write loops forever, this announcer persists nothing. ⚠ The moment anything on that path writes a document — an outbox row, a delivery receipt — the guard becomes mandatory. It deliberately does **not** batch per request, unlike `ProfileStatusAnnouncer`: that frame is a snapshot, this one is a log, and "the row changed twice" is the fact it exists to carry. Its known holes are in its javadoc — query-based updates (`updateFirst`/`updateMulti`/`findAndModify`) raise no event, a criteria delete names no single `_id`, and a first write with a **caller-assigned id** is reported `UPDATED`, because `save` is an upsert and nothing distinguishes it from a replacement.
